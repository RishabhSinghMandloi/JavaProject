...
public class Utility{
	...
	/**
	 * method to get integer type value from the user
	 * @return
	 */
	...
	/**
	 * method to display string type array
	 * @param arr
	 */
	public static void dispalyStringArray(String arr[])
	{
		for(int i=0;i<arr.length;i++)
		{
			System.out.println(arr[i]);
	...
	/**
	 * method to search the word in the file 
	 * @param path
	 */
	public static void searchWordFromFile(String path)
	{
		...
		try {
			...
			int index = Utility.binarySearchString(arr, 0, arr.length-1, word);
			if(index < 0)
	 ...
	 * 
	 * @param number
	 * @return double type square root
	 */
	public static double sqrt(double number)
	{
		double epsillion = 1e-15;
		double t=number;
		while(Math.abs(t-(number/t))>epsillion*t)
		{
			t=((number/t)+t)/2;
		}
		return t;
	...
	public String decimalToAny(int n,int base)
	{
		int ch;
		String s="";
		while(n!=0)
	...
	public static void displayArrayofString(String arr[])
	{
	   for(int i=0;i<arr.length;i++)
	   {
		   System.out.println(arr[i]);
	...
	{
      System.out.println("Enter how many Strings input do you want"); 
      int n=sc.nextInt();
	...
	public static void mergeString(String arr[],int low,int mid,int high)
	{
		 int n1=mid-low+1;
		 int n2=high-mid;
		 String t1[]=new String[n1];
		 String t2[]=new String[n2];
		 for(int i=0;i<n1;i++)
		 {
			 t1[i]=arr[low+i]; 
		 }
		 ...
		 while(i<n1&&j<n2)
		 {
			 if(t1[i].compareToIgnoreCase(t2[j])<0)
			 {
				 arr[k++]=t1[i++];
			 }else {
				 arr[k++]=t2[j++];
	...
	public static void mergeSortString(String arr[],int low,int high)
	{
		if(low<high)
		{
			int mid=(low+high)/2;
